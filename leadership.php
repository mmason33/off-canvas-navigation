<?php
/**
 * Template Name: Legacy of Leadership
 */
?>

<?php get_template_part('includes/half-n-half'); ?>
<div class="container visible-xs-down hidden-sm-up">
<div class="row leadership-mobile">
  <div class="col-xs-12">
      <p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged.</p>
  </div>
</div>
</div>
<div class="container-fluid">
  <div class="row leadership-grid">
  <div class="col-lg-3 col-md-3 col-sm-3 leadership-image">
    <img src="/wp-content/uploads/2017/03/paul-400x400.jpg" class="img-fluid">
    <div class="visible-xs-down hidden-sm-up">
    <h3>Paul Alexander</h3>
    <h4>President and Founder</h4>
    <a href="#">Read Bio</a>
    </div>
  </div>
  <div class="col-lg-9 col-md-9 col-sm-9 hidden-xs-down">
    <div class="leadership-bio">
    <h3>Paul Alexander</h3>
    <h4>President and Founder</h4>
    <p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.
    </p>
  </div>
  </div>
  <div class="col-lg-9 col-md-9 col-sm-9 hidden-xs-down">
    <div class="leadership-bio">
    <h3>Michael Netherton</h3>
    <h4>Chief Executive Officer</h4>
    <p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.
    </p>
  </div>
  </div>
  <div class="col-lg-3 col-md-3 col-sm-3 leadership-image text-lg-right">
    <img src="/wp-content/uploads/2017/03/netherton-400x400.jpg" class="img-fluid">
    <div class="visible-xs-down hidden-sm-up">
    <h3>Michael Netherton</h3>
    <h4>Chief Executive Officer</h4>
    <a href="#">Read Bio</a>
    </div>
  </div>
  <div class="col-lg-3 col-md-3 col-sm-3 leadership-image">
    <img src="/wp-content/uploads/2017/03/kaufman-400x400.jpg" class="img-fluid">
    <div class="visible-xs-down hidden-sm-up">
    <h3>Dr. Edward Kaufman</h3>
    <h4>Medical Director</h4>
    <a href="#">Read Bio</a>
    </div>
  </div>
  <div class="col-lg-9 col-md-9 col-sm-9 hidden-xs-down">
    <div class="leadership-bio">
    <h3>Dr. Edward Kaufman</h3>
    <h4>Medical Director</h4>
    <p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.
    </p>
  </div>
  </div>
  <div class="col-lg-9 col-md-9 col-sm-9 hidden-xs-down">
    <div class="leadership-bio">
    <h3>Illana Zukavich</h3>
    <h4>Clinical Director</h4>
    <p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.
    </p>
  </div>
  </div>
  <div class="col-lg-3 col-md-3 col-sm-3 leadership-image text-lg-right">
    <img src="/wp-content/uploads/2017/03/illana-400x400.jpg" class="img-fluid">
    <div class="visible-xs-down hidden-sm-up">
    <h3>Illana Zuchavich</h3>
    <h4>Clinical Director</h4>
    <a href="#">Read Bio</a>
    </div>
  </div>
  <div class="col-lg-3 col-md-3 col-sm-3 leadership-image">
    <img src="/wp-content/uploads/2017/03/kevin-leadership-400x400-1.jpg" class="img-fluid">
    <div class="visible-xs-down hidden-sm-up">
    <h3>Kevin McCauley</h3>
    <h4>Senior Recovery Advisor</h4>
    <a href="#">Read Bio</a>
    </div>
  </div>
  <div class="col-lg-9 col-md-9 col-sm-9 hidden-xs-down">
    <div class="leadership-bio">
    <h3>Kevin McCauley</h3>
    <h4>Senior Recovery Advisor</h4>
    <p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.
    </p>
  </div>
  </div>
  <div class="col-lg-9 col-md-9 col-sm-9 hidden-xs-down">
    <div class="leadership-bio">
    <h3>Devon Wayt</h3>
    <h4>Vice President of Operations</h4>
    <p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.
    </p>
  </div>
  </div>
  <div class="col-lg-3 col-md-3 col-sm-3 leadership-image text-lg-right">
    <img src="/wp-content/uploads/2017/03/devon-400x400.jpg" class="img-fluid">
    <div class="visible-xs-down hidden-sm-up">
    <h3>Devon Wayt</h3>
    <h4>Vice President of Operations</h4>
    <a href="#">Read Bio</a>
    </div>
  </div>
  <div class="col-lg-3 col-md-3 col-sm-3 leadership-image">
    <img src="/wp-content/uploads/2017/03/ray-400x400.jpg" class="img-fluid">
    <div class="visible-xs-down hidden-sm-up">
    <h3>Ray Pacini</h3>
    <h4>Chief Financial Officer</h4>
    <a href="#">Read Bio</a>
    </div>
  </div>
  <div class="col-lg-9 col-md-9 col-sm-9 hidden-xs-down">
    <div class="leadership-bio">
    <h3>Ray Pacini</h3>
    <h4>Chief Financial Officer</h4>
    <p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.
    </p>
  </div>
  </div>
  <div class="col-lg-9 col-md-9 col-sm-9 hidden-xs-down">
    <div class="leadership-bio">
      <h3>Dustin Frei</h3>
      <h4>Vice President of Business Development</h4>
      <p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.
      </p>
  </div>
  </div>
  <div class="col-lg-3 col-md-3 col-sm-3 leadership-image text-lg-right">
      <img src="/wp-content/uploads/2017/03/dustin-400x400.jpg" class="img-fluid">
      <div class="visible-xs-down hidden-sm-up">
      <h3>Dustin Frei</h3>
      <h4>Vice President of Business Development</h4>
      <a href="#">Read Bio</a>
      </div>
  </div>
  <div class="col-lg-3 col-md-3 col-sm-3 leadership-image">
    <img src="/wp-content/uploads/2017/03/coach-400x400.jpg" class="img-fluid">
    <div class="visible-xs-down hidden-sm-up">
    <h3>Emily Bielen</h3>
    <h4>Vice President of Referral Relations</h4>
    <a href="#">Read Bio</a>
    </div>
  </div>
  <div class="col-lg-9 col-md-9 col-sm-9 hidden-xs-down">
    <div class="leadership-bio">
      <h3>Emily Bielen</h3>
      <h4>Vice President of Referral Relations</h4>
      <p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.
    </p>
    </div>
  </div>
  </div>
</div>
